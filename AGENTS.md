...
**Бэк — в этой же папке** (`contacts-service/`, 2026-08-24, переехал из
`server_back/contacts-service/`) — на отдельной ветке `backend` (main — чистый
...
- **«Фиксируй» = поднять версию (+0.0.1 в `manifest.json` и `package.json`), обновить
  ...
  чисто бэковых изменений в `contacts-service/` (ветка `backend`), если сам плагин
  не менялся. Без явного подтверждения пользователя
